...
app = FastAPI(
    ...
    summary="A sample application showing how to use Kubernetes with an API Service and an external repository"
)
...
mongo_host = os.getenv('MONGO_HOST', 'mongodb-service:27017')
mongo_db_name = os.getenv('MONGO_DB', 'test')

mongo_connection = "mongodb://{mongo_user}:{mongo_password}@{mongo_host}/{mongo_db_name}?authSource=admin"

# Crear el cliente de MongoDB
mongo_client = MongoClient(mongo_connection)
coll = mongo_client.get_database(mongo_db_name).get_collection('clients')
